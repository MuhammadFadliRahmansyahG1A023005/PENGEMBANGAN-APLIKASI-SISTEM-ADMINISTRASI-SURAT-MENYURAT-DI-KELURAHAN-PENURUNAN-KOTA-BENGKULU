...
<head>
    <meta charset="utf-8">
    <title>SKU - Kelurahan Penurunan</title>
    <style>
        body {
...
<body>
    ...
    {{-- DATA PERSONAL (Sesuai field SKU di database Anda) --}}
    <table class="data-table">
        ...
        <tr>
            ...
            <td>:</td>
            <td>
                {{ \Carbon\Carbon::parse($surat->masaberlakuawal)->isoFormat('DD MMMM YYYY') }}
                s/d
                {{ \Carbon\Carbon::parse($surat->masaberlakusampai)->isoFormat('DD MMMM YYYY') }}
            </td>
        </tr>
    </table>
    ...
    {{-- KETERANGAN USAHA --}}
    <div class="isi-surat">
        Berdasarkan Surat Pengantar RT No.100/580/RT.01/RW.01/2026 tanggal
        {{ \Carbon\Carbon::parse($surat->created_at)->isoFormat('DD MMMM YYYY') }} orang tersebut diatas mempunyai
        Usaha <b>{{ $surat->namausaha ?? 'Warung Manisan' }}</b> di {{ $surat->lokasiusaha }}. Surat
        keterangan ini dibuat untuk {{ $surat->kebutuhan ? $surat->kebutuhan : 'keperluan Kelengkapan Pinjaman KUR BRI' }}.
    </div>

    <div class="isi-surat">
        Demikian surat keterangan usaha ini dibuat dengan sebenarnya agar dapat dipergunakan sebagaimana mestinya.
    </div>

    {{-- TANDA TANGAN (Style sesuai gambar baru) --}}
